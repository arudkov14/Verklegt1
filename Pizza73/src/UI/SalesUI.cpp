...
//    int pizza_price;
  ...
  //    cout << "2: add beverage" << endl;
        ...
        if(input == '0') {
            ...
            thisorder = Order(name, total_price, comment, order_status, pizzas, payment_status);
...
int SalesUI::get_total_pizza_price(vector<Pizza> pizzas) {
    ...
    for(unsigned int i = 0; i < pizzas.size(); i++) {
        ...
        TotalPrice += thispizzasize.get_base_price();
        }
    return TotalPrice;
...
int SalesUI::get_size_price(PizzaSize pizzasize) {
    int sizeprice = pizzasize.get_base_price();
